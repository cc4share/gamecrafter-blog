...
在 Notion「想法」数据源勾选「首页展示」，首页会按发布日期倒序选取最多 5 条已发布想法。取消勾选、改为草稿或关闭想法栏目后，对应内容不会出现在首页轮播；全部取消时隐藏轮播。它独立于栏目管理中的「首页展示」，不会把想法混入「最近更新」。

轮播优先使用正文第一段文字，其次使用摘要或标题；最多显示两行，点击查看完整想法。每 10 秒切换，也可点击换一条图标；鼠标悬停、键盘聚焦、页面不在前台或轮播不在视野内时暂停。系统开启减少动态效果时只手动切换。桌面和手机均使用名字在上、小头像与想法在下一行的布局。
